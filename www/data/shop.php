...
$index = mysql_query("select * from fs_shop", $db);
while ($artikel_arr = mysql_fetch_assoc($index))
{
    ...
    $index2 = mysql_query("select shop_artikel from fs_template where id = '$global_config_arr[design]'", $db);
...
$index = mysql_query("select shop_main_body from fs_template where id = '$global_config_arr[design]'", $db);
