...
for ( $i = $config_arr['page_start']; $i < $maximum; $i++ )
{
    ...
    $line_template->tag ( 'user_image', ( image_exists ( 'media/user-images/', $col_id[$i] ) ) ? '<img src="'.image_url ( 'media/user-images/', $col_id[$i] ).'" alt="'.$FD->text("frontend", "user_image_of").' '.kill_replacements ( $col_name[$i], TRUE ).'">' : $FD->text("frontend", "user_image_not_found") );
