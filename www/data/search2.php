...
// There is a search
    ...
    $search = new Search("news", $sq->getTree());
    var_dump($search->getResult()->getSet());


    $key = $_REQUEST['keyword'];
    unset($_REQUEST);
    $_REQUEST['keyword'] = $key;
	...
	if ($_REQUEST['in_downloads']) {
		...
		while($found = $search->next() && $config_arr['search_num_previews'] > $downloads_num_results) {
			...
			if (!empty($dl)) {
				...
				$template->tag("title", $dl['dl_title']);
