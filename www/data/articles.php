...
if ($FD->cfg('goto') == 'articles') {
    ...
    $article_arr = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'articles WHERE article_id = '.intval($_GET['id']));
...
// Article exists
    ...
    $user_arr = $FD->db()->conn()->query('SELECT user_id, user_name FROM '.$FD->config('pref').'user WHERE user_id ='.intval($article_arr['article_user']).' LIMIT 1');
