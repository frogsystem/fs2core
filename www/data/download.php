...
if ($index)
{
    ...
    {
        if (strlen($dl_arr[dl_text]) > 150)
           $dl_arr[dl_text] = substr($dl_arr[dl_text], 0, 150) . "...";
        $dl_arr[dl_date] = date("d.m.Y" , $dl_arr[dl_date]);
        $index3 = mysql_query("select cat_name from fs_dl_cat where cat_id = '$dl_arr[cat_id]'", $db);
        $dl_arr[cat_name] = stripslashes(mysql_result($index3, 0, "cat_name"));
        $index2 = mysql_query("select dl_datei_preview from fs_template where id = '$global_config_arr[design]'", $db);
...
$index = mysql_query("select dl_body from fs_template where id = '$global_config_arr[design]'", $db);
...
$template = str_replace("{aktiv$_GET[catid]}", 2, $template);
$template = str_replace("{aktiv1}", "", $template);
$template = str_replace("{aktiv0}", "", $template);
