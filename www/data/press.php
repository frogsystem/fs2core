...
if (!($config_arr['game_navi'] == 0 && $config_arr['cat_navi'] == 0 && $config_arr['lang_navi'] == 0))
{
    ...
    { //Spiele anzeigen
        $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'press_admin WHERE type = 1 ORDER BY title ASC');
        while ($game_arr = $index->fetch(PDO::FETCH_ASSOC))
        {
            $select = $FD->db()->conn()->query('SELECT COUNT(press_id) AS number FROM '.$FD->config('pref').'press WHERE press_game = '.intval($game_arr['id']) );
            ...
            { //Es existieren Presseberichte
                ...
                { //Kategorien & Spiele anzeigen
                    $index2 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'press_admin WHERE type = 2 ORDER BY title ASC');
                    while ($cat_arr = $index2->fetch(PDO::FETCH_ASSOC))
                    {
                        $select = $FD->db()->conn()->query('SELECT COUNT(press_id) AS number FROM '.$FD->config('pref')."press WHERE press_game = $game_arr[id] AND press_cat = $cat_arr[id]");
                        if ($select->fetchColumn() > 0)
                        { //Es existieren Presseberichte
                            $navi_arr[][] = $cat_arr['id'];
                            ...
                            { //Sprachen & Kategorien & Spiele anzeigen
                                $index3 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'press_admin WHERE type = 3 ORDER BY title ASC');
                                while ($lang_arr = $index3->fetch(PDO::FETCH_ASSOC))
                                {
                                    $select = $FD->db()->conn()->query('SELECT COUNT(press_id) AS number FROM '.$FD->config('pref')."press WHERE press_game = $game_arr[id] AND press_cat = $cat_arr[id] AND press_lang = $lang_arr[id]");
                                    if ($select->fetchColumn() > 0)
                                    { //Es existieren Presseberichte
                ...
                { //Sprachen & Spiele anzeigen
                    $index1 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'press_admin WHERE type = 3 ORDER BY title ASC');
                    while ($lang_arr = $index1->fetch(PDO::FETCH_ASSOC))
                    {
                        $select = $FD->db()->conn()->query('SELECT COUNT(press_id) AS number FROM '.$FD->config('pref')."press WHERE press_game = $game_arr[id] AND press_lang = $lang_arr[id]" );
                        if ($select->fetchColumn() > 0)
                        {
                            $navi_arr[][] = $lang_arr['id'];
                            $last_arr[] = $lang_arr['id'];
    ...
    { //Kategorien anzeigen
        $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'press_admin WHERE type = 2 ORDER BY title ASC');
        while ($cat_arr = $index->fetch(PDO::FETCH_ASSOC))
        {
            $select = $FD->db()->conn()->query('SELECT COUNT(press_id) AS number FROM '.$FD->config('pref')."press WHERE press_cat = $cat_arr[id]");
            if ($select->fetchColumn() > 0)
            { //Es existieren Presseberichte
                $navi_arr[] = $cat_arr['id'];
                ...
                { //Sprachen & Kategorien anzeigen
                    $index2 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'press_admin WHERE type = 3 ORDER BY title ASC');
                    while ($lang_arr = $index2->fetch(PDO::FETCH_ASSOC))
                    {
                        $select = $FD->db()->conn()->query('SELECT COUNT(press_id) AS number FROM '.$FD->config('pref')."press WHERE press_cat = $cat_arr[id] AND press_lang = $lang_arr[id]");
                        if ($select->fetchColumn() > 0)
                        { //Es existieren Presseberichte
                            $navi_arr[][] = $lang_arr['id'];
                            $last_arr[] = $lang_arr['id'];
    ...
    elseif ($config_arr['lang_navi'] != 0)
    { //Sprachen anzeigen
        $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'press_admin WHERE type = 3 ORDER BY title ASC');
        while ($lang_arr = $index->fetch(PDO::FETCH_ASSOC))
        {
            $select = $FD->db()->conn()->query('SELECT COUNT(press_id) AS number FROM '.$FD->config('pref')."press WHERE press_lang = $lang_arr[id]");
            if ($select->fetchColumn() > 0)
            { //Es existieren Presseberichte
                $navi_arr[] = $lang_arr['id'];
                $last_arr[] = $lang_arr['id'];
    ...
    if ( count ( $navi_arr ) > 0 )
    {
        foreach ($navi_arr as $value)
        {
            if (is_array($value) AND $open == true)
            {
                foreach ($value as $value2)
                {
                    if (is_array($value2) AND $open2 == true)
                    {
                        ...
                        {
                            $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."press_admin WHERE id = $value3");
                    ...
                    {
                        $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."press_admin WHERE id = $value2");
                        $entry_arr = $index->fetch(PDO::FETCH_ASSOC);
            ...
            {
                $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."press_admin WHERE id = $value");
                $entry_arr = $index->fetch(PDO::FETCH_ASSOC);
...
if ($config_arr['show_press_sql'] == true)
{
    ...
    while ($press_arr = $index->fetch(PDO::FETCH_ASSOC))
    {
        ...
        $index2 = $FD->db()->conn()->query('SELECT title FROM '.$FD->config('pref')."press_admin WHERE id = '$press_arr[press_game]'");
        $press_arr['press_game_title'] = $index2->fetchColumn();
        $index2 = $FD->db()->conn()->query('SELECT title FROM '.$FD->config('pref')."press_admin WHERE id = '$press_arr[press_cat]'");
        $press_arr['press_cat_title'] = $index2->fetchColumn();
        $index2 = $FD->db()->conn()->query('SELECT title FROM '.$FD->config('pref')."press_admin WHERE id = '$press_arr[press_lang]'");
