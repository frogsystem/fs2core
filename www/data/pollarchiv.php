...
if ($_GET[pollid])
{
    ...
    $index = mysql_query("select * from ".$global_config_arr[pref]."poll where poll_id = $_GET[pollid]", $db);
    ...
    // Antworten auswerten
    $index = mysql_query("select * from ".$global_config_arr[pref]."poll_answers where poll_id = $_GET[pollid]", $db);
...
else
{
        ...
        switch($_GET['sort']) {
                case 'name_desc': {
                        $index = mysql_query("select * from ".$global_config_arr[pref]."poll order by poll_quest desc", $db);
                        $order_name = 'asc';
                        $arrow_name = '<img src="images/icons/pointer_down.gif" alt="" border="0" title="Absteigend" />';
                        break;
                }
                case 'name_asc': {
                        $index = mysql_query("select * from ".$global_config_arr[pref]."poll order by poll_quest asc", $db);
                        $order_name = 'desc';
                        $arrow_name = '<img src="images/icons/pointer_up.gif" alt="" border="0" title="Aufsteigend" />';
                        break;
                }
                case 'voters_desc': {
                        $index = mysql_query("select * from ".$global_config_arr[pref]."poll order by poll_participants desc", $db);
                        $order_voters = 'asc';
                        $arrow_voters = '<img src="images/icons/pointer_down.gif" alt="" border="0" title="Absteigend" />';
                        break;
                }
                case 'voters_asc': {
                        $index = mysql_query("select * from ".$global_config_arr[pref]."poll order by poll_participants asc", $db);
                        $order_voters = 'desc';
                ...
                case 'startdate_asc': {
                        $index = mysql_query("select * from ".$global_config_arr[pref]."poll order by poll_start asc", $db);
                        $order_startdate = 'desc';
                        $arrow_startdate = '<img src="images/icons/pointer_up.gif" alt="" border="0" title="Aufsteigend" />';
                        break;
                }
                case 'enddate_desc': {
                        $index = mysql_query("select * from ".$global_config_arr[pref]."poll order by poll_end desc", $db);
                        $order_enddate = 'asc';
                        $arrow_enddate = '<img src="images/icons/pointer_down.gif" alt="" border="0" title="Absteigend" />';
                        break;
                }
                case 'enddate_asc': {
                        $index = mysql_query("select * from ".$global_config_arr[pref]."poll order by poll_end asc", $db);
                        $order_enddate = 'desc';
    ...
    $index = mysql_query("select poll_list from ".$global_config_arr[pref]."template where id = '$global_config_arr[design]'", $db);
