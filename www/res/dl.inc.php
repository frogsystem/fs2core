...
if ($_GET[fileid] && $_GET[dl])
{
    ...
    //Is Mirror?
    $index = mysql_query("select file_is_mirror from ".$global_config_arr[pref]."dl_files where file_id = '$_GET[fileid]'", $db);
        ...
        OR $check_file_is_mirror == 1)
    {
      ...
      $index = mysql_query("select * from ".$global_config_arr[pref]."dl_files where file_id = $_GET[fileid]", $db);
      $file = mysql_fetch_array($index);
      mysql_query("update ".$global_config_arr[pref]."dl_files set file_count=file_count+1 where file_id = $file[file_id]", $db);
