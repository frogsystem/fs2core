...
$permanent = mysql_query("SELECT partner_id, 
                                 ...
                                 partner_permanent
                          FROM ".$global_config_arr[pref]."partner WHERE partner_permanent = 1", $db);
while ($permanent_arr = mysql_fetch_assoc($permanent))
{
    $index2 = mysql_query("select partner_navi_eintrag from ".$global_config_arr[pref]."template where id = '$global_config_arr[design]'", $db);
...
$index = mysql_query("SELECT partner_anzahl FROM ".$global_config_arr[pref]."partner_config", $db);
...
$index = mysql_query("SELECT COUNT(partner_id) AS number FROM ".$global_config_arr[pref]."partner", $db);
if (mysql_result($index,0,"number")<$rand_arr[shuffle_all]){
    $rand_arr[shuffle_all] = mysql_result($index,0,"number");
}
$index = mysql_query("SELECT COUNT(partner_id) AS number FROM ".$global_config_arr[pref]."partner WHERE partner_permanent = 0", $db);
...
$non_permanent = mysql_query("SELECT partner_id,
                                     ...
                                     partner_beschreibung,
                                     partner_permanent
                              FROM ".$global_config_arr[pref]."partner WHERE partner_permanent = 0", $db);
...
$index = mysql_query("select partner_navi_body from ".$global_config_arr[pref]."template where id = '$global_config_arr[design]'", $db);
...
unset($partner_all_list);
unset($permanent_list);
unset($non_permanent_list);
?>