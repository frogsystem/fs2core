...
$index = mysql_query("select artikel_id, artikel_name, artikel_url from fs_shop where artikel_hot = 1", $db);
while ($shop_arr = mysql_fetch_assoc($index))
{
   $index2 = mysql_query("select shop_hot from fs_template where id = '$global_config_arr[design]'", $db);
...
$index = mysql_query("select shop_body from fs_template where id = '$global_config_arr[design]'", $db);
