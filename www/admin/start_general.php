...
// Online since
$online_since = $FD->db()->conn()->query('SELECT s_year, s_month, s_day FROM `'.$FD->config('pref').'counter_stat` ORDER BY `s_year`, `s_month`, `s_day` LIMIT 1');
...
// Total
$total = $FD->db()->conn()->query('SELECT visits, hits FROM `'.$FD->config('pref').'counter` LIMIT 1');
$total = $total->fetch(PDO::FETCH_ASSOC);
// Today
$today = $FD->db()->conn()->query('SELECT s_hits, s_visits FROM `'.$FD->config('pref').'counter_stat` WHERE `s_year` = \''.(int) $FD->env('year')."' AND `s_month` = '".(int) $FD->env('month')."' AND `s_day` = '".(int) $FD->env('day')."' LIMIT 1");
...
// Referrer Num
$index = $FD->db()->conn()->query("SELECT COUNT(`ref_url`) AS 'ref_num' FROM ".$FD->config('pref')."counter_ref");
$ref['num'] = $index->fetchColumn();

if ($ref['num'] > 0) {
	// last Ref
	$index = $FD->db()->conn()->query('SELECT ref_url, ref_last FROM '.$FD->config('pref').'counter_ref ORDER BY ref_last DESC LIMIT 0,1');
