...
if (isset($_POST['dledit']) && isset($_POST['title']) && isset($_POST['text']) && !isset($_POST['files_add']))
{
    ...
    {
        $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."dl WHERE dl_id = '$_POST[editdlid]'");
        $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."dl_files WHERE dl_id = '$_POST[editdlid]'");
    ...
    else
    {
        ...
        $stmt = $FD->db()->conn()->prepare('UPDATE '.$FD->config('pref')."dl
        ...
        for ($i=0; $i<count($_POST['fname']); $i++)
        {
            if (isset($_POST['delf'][$i]) && $_POST['delf'][$i]!=0)
            {
                ...
                $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref').'dl_files WHERE file_id = ' . $_POST['delf'][$i]);
            }
            else
            {
                ...
                {
                    $stmt = $FD->db()->conn()->prepare('UPDATE '.$FD->config('pref')."dl_files
...
if (isset($_POST['dlid']) || isset($_POST['dlid']) && isset($_POST['files_add']) && isset($_POST['optionsadd']))
{
    ...
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."dl WHERE dl_id =  '$_POST[dlid]'");
    ...
    $index = $FD->db()->conn()->query('SELECT COUNT(*) FROM '.$FD->config('pref')."dl_files WHERE dl_id = '$_POST[dlid]'");
    $rows = $index->fetchColumn();
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."dl_files WHERE dl_id = '$_POST[dlid]' ORDER BY file_id");
    for($i=0; $i<$rows; $i++)
    {
        $row = $index->fetch(PDO::FETCH_ASSOC);
    ...
    echo '<form action="" method="post">
          ...
          <table border="0" cellpadding="4" cellspacing="0" width="600" class="content">
              ...
              ';
    $comments = $FD->db()->conn()->query('SELECT COUNT(*) FROM `'.$FD->config('pref').'comments` WHERE content_type=\'dl\' AND content_id=\''.$_POST['dlid']."'");
    ...
    else
    {
      ...
      $comments = $FD->db()->conn()->query('SELECT * FROM `'.$FD->config('pref').'comments` WHERE content_type=\'dl\' AND content_id=\''.$_POST['dlid']."'");
      while ($row = $comments->fetch(PDO::FETCH_ASSOC))
      {
        ...
        {
          $user = $FD->db()->conn()->query('SELECT user_name FROM `'.$FD->config('pref')."user` WHERE user_id='".$row['comment_poster_id']."' LIMIT 1");
...
else
{
    ...
    $index = $FD->db()->conn()->query('SELECT dl_id, dl_name, cat_id FROM '.$FD->config('pref')."dl $wherecat ORDER BY dl_name");
    while ($dl_arr = $index->fetch(PDO::FETCH_ASSOC))
    {
        $catindex = $FD->db()->conn()->query('SELECT cat_name FROM '.$FD->config('pref')."dl_cat WHERE cat_id = '$dl_arr[cat_id]'");
        $dbcatname = $catindex->fetchColumn();
        
        $comments = $FD->db()->conn()->query('SELECT COUNT(*) FROM `'.$FD->config('pref').'comments` WHERE content_type=\'dl\' AND content_id=\''.$dl_arr['dl_id']."'");
