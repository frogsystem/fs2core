   ...
   )
{
    ...
    {
        $index = $FD->db()->conn()->query('SELECT type FROM '.$FD->config('pref')."press_admin WHERE id = '$_POST[entry_id]'");
        ...
        $FD->db()->conn()->exec('UPDATE '.$FD->config('pref').'press '.$entry_arr['type_set'].' '.$entry_arr['type_where']);

        $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."press_admin
   ...
   )
{
    $_POST['entry_id'] = $_POST['entry_id'][0];
    settype($_POST['entry_id'], 'integer');
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."press_admin WHERE id = $_POST[entry_id]");
   ...
   )
{
    $_POST['entry_id'] = $_POST['entry_id'][0];
    settype($_POST['entry_id'], 'integer');
    $index = $FD->db()->conn()->query('SELECT COUNT(id) AS number FROM '.$FD->config('pref').'press_admin
...
if (!isset($_POST['entry_id']))
{
    ...
    for ($i=1;$i<=3;$i++)
    {
        $index = $FD->db()->conn()->query('SELECT COUNT(*) FROM '.$FD->config('pref')."press_admin WHERE type='$i'");
        ...
        $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."press_admin WHERE type='$i' ORDER BY type, title");
