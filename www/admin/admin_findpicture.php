...
if (isset($_POST['cat']))
{
    $index = $FD->db()->conn()->prepare('SELECT COUNT(*) FROM '.$FD->config('pref').'screen WHERE cat_id = ?');
    ...
    // found images
        ...
        $index = $FD->db()->conn()->prepare('SELECT * FROM '.$FD->config('pref').'screen WHERE cat_id = ? ORDER BY screen_id DESC');
        $index->execute(array($_POST['cat']));
