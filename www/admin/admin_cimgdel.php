...
if(isset($_GET['file'])){
    ...
    $qry = $FD->db()->conn()->query('SELECT * FROM `'.$FD->config('pref').'cimg` WHERE `id`='.$file);
    ...
    if($row !== false){
        if(file_exists(CIMG_PATH."{$row['name']}.{$row['type']}")){
            if(!isset($_POST['edit'])){ // edit file
                        ...
                        $qry = $FD->db()->conn()->query('SELECT * FROM `'.$FD->config('pref').'cimg_cats`');
...
FS2_STRING;
                ...
                if(isset($_POST['delete'])){
                    ...
                    $FD->db()->conn()->exec('DELETE FROM `'.$FD->config('pref').'cimg` WHERE `id`='.intval($file));
                    ...
                    if($_POST['name'] != $row['name']){
                        ...
                        $stmt = $FD->db()->conn()->prepare('UPDATE `'.$FD->config('pref').'cimg` SET `name`= ? WHERE `id`='.intval($file));
                        $stmt->execute(array($_POST['name']));
                    }

                    if($_POST['cat'] != $row['cat']){
                        $FD->db()->conn()->exec('UPDATE `'.$FD->config('pref')."cimg` SET `cat`='".intval($_POST['cat'])."' WHERE `id`=".$file);
                    ...
                    if(isset($_POST['thumb']) && !empty($_POST['width']) && !empty($_POST['height'])){
                        ...
                        $FD->db()->conn()->exec('UPDATE `'.$FD->config('pref').'cimg` SET `hasthumb`=1 WHERE `id`='.$file);
            ...
            $FD->db()->conn()->exec('DELETE FROM `'.$FD->config('pref').'cimg` WHERE `id`='.$file);
...
if(!isset($_GET['file'])){ // select file
    $num = $FD->db()->conn()->exec('UPDATE `'.$FD->config('pref').'cimg` SET `cat` = 0 WHERE `cat` != 0 AND `cat` NOT IN (SELECT DISTINCT `id` FROM `'.$FD->config('pref').'cimg_cats`)');
    if ($num == 1) {
        systext('Ein Datensatz wurde automatisch korrigiert.');
    } elseif($num > 1) {
        systext($num.' Datens&auml;tze wurden automatisch korrigiert.');
    }
    $qry = $FD->db()->conn()->query('SELECT COUNT(*) FROM `'.$FD->config('pref').'cimg`');
...
FS2_STRING;
        ...
        $first = true;
        $qry = $FD->db()->conn()->query('SELECT * FROM `'.$FD->config('pref').'cimg` ORDER BY `cat`');
        while(($row = $qry->fetch(PDO::FETCH_ASSOC)) !== false) {
            if($row['cat'] != $actcat['id']){
                $qry2 = $FD->db()->conn()->query('SELECT * FROM `'.$FD->config('pref').'cimg_cats` WHERE `id`='.intval($row['cat']));
