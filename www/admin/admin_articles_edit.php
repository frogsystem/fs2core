...
function default_display_filter ( $FORM )
{
	...
	echo'
                                    ...
                                    <select name="order">
                                        <option value="article_title" '.getselected ( "article_title", $FORM['order'] ).'>'.$admin_phrases[news][news_edit_filter_id].'</option>
                                        <option value="article_date" '.getselected ( "article_date", $FORM['order'] ).'>'.$admin_phrases[news][news_edit_filter_date].'</option>
                                        <option value="article_url" '.getselected ( "article_url", $FORM['order'] ).'>'.$admin_phrases[news][news_edit_filter_newstitle].'</option>
                                    </select>,
                                    <select name="sort">
...
function default_display_pagenav ( $pagenav_arr )
{
    ...
    {
        $next_page = '<a href="'.$PHP_SELF.'?mid=content&go=articlesedit&order='.$_REQUEST['order'].'&sort='.$_REQUEST['sort'].'&cat_id='.$_REQUEST['cat_id'].'&start='.$pagenav_arr['new_start'].'">'.$admin_phrases[news][news_edit_next_news].' »</a>';
    }
    if ( $pagenav_arr['old_start_exists'] )
    {
        $prev_page = '<a href="'.$PHP_SELF.'?mid=content&go=articlesedit&order='.$_REQUEST['order'].'&sort='.$_REQUEST['sort'].'&cat_id='.$_REQUEST['cat_id'].'&start='.$pagenav_arr['old_start'].'">« '.$admin_phrases[news][news_edit_prev_news].'</a>';
    }
...
function default_get_entry_data ( $articles_arr )
{
	...
	if ( $articles_arr['article_user'] != 0 ) {
	    ...
	    $articles_arr['user_name'] = "unbekannt";
	}
...
function action_edit_get_data ( $ARTICLE_ID )
{
    ...
    // Get User
	if ( $articles_arr['article_user'] != 0 ) {
		$index = mysql_query ( "SELECT user_name, user_id FROM ".$global_config_arr['pref']."user WHERE user_id = '".$articles_arr['article_user']."'", $db );
...
function action_delete_get_data ( $ARTICLE_ID )
{
	...
	$index = mysql_query ( "SELECT * FROM ".$global_config_arr['pref']."articles WHERE article_id = '".$ARTICLE_ID."'", $db );
	$articles_arr = mysql_fetch_assoc ( $index );
						...
						<table class="configtable" cellpadding="4" cellspacing="0">
							...
							<tr>
                                <td class="config right">
                                    <a href="'.$global_config_arr['virtualhost'].'?go=articles&id='.$articles_arr['article_id'].'" target="_blank">» '.$admin_phrases[news][news_delete_view_news].'</a></div>
                                </td>
							</tr>
							<tr><td class="space"></td></tr>
						</table>
