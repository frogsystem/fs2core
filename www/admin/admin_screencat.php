...
if (isset($_POST['cat_id']) && isset($_POST['cat_name']) && !emptystr($_POST['cat_name']) && $_POST['sended'] == 'edit')
{
    ...
    $stmt = $FD->db()->conn()->prepare('UPDATE '.$FD->config('pref')."screen_cat
                 SET cat_name = ?,
...
elseif (isset($_POST['cat_id']) && isset($_POST['sended']) && $_POST['sended'] == 'delete')
{
  ...
  $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."screen_cat
               WHERE cat_id = '$_POST[cat_id]'");

  $FD->db()->conn()->exec('UPDATE '.$FD->config('pref')."screen
...
/////////////////////
  ...
  {
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."screen_cat WHERE cat_id = '$_POST[cat_id]'");
...
///////////////////////
  ...
  elseif ($_POST['cat_action'] == 'delete')
  {
    ...
    $index = $FD->db()->conn()->query('SELECT COUNT(*) AS categ_count FROM '.$FD->config('pref')."screen_cat WHERE cat_id != '$admin_cat_arr[cat_id]' AND cat_type = '$admin_cat_arr[cat_type]'" );
    $num_rows = $index->fetchColumn();

    if ($num_rows > 0)
    {
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."screen_cat WHERE cat_id != '$admin_cat_arr[cat_id]' AND cat_type = '$admin_cat_arr[cat_type]' ORDER BY cat_name" );
...
else
{
    ...
    ';
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'screen_cat ORDER BY cat_date DESC');
    while ($cat_arr = $index->fetch(PDO::FETCH_ASSOC))
    {
        $cat_arr['cat_date'] = date('d.m.Y', $cat_arr['cat_date']);

        if ( $cat_arr['cat_type'] == 2 ) {
            $number_index = $FD->db()->conn()->query('SELECT COUNT(wallpaper_id) AS number FROM '.$FD->config('pref')."wallpaper WHERE cat_id = $cat_arr[cat_id]");
        } else {
            $number_index = $FD->db()->conn()->query('SELECT COUNT(screen_id) AS number FROM '.$FD->config('pref')."screen WHERE cat_id = $cat_arr[cat_id]");
        }
