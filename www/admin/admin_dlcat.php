...
if (isset($_POST['catname']))
{
    ...
    {
        $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref').'dl_cat WHERE cat_id = '.$_POST['catid']);
...
if (isset($_POST['editcatid']))
{
    ...
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."dl_cat WHERE cat_id = '$_POST[editcatid]'");
...
else
{
    ...
    ';
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'dl_cat ORDER BY cat_name');
