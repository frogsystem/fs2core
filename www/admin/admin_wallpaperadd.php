...
$index = $FD->db()->conn()->prepare('SELECT COUNT(*) AS wp_count FROM '.$FD->config('pref').'wallpaper WHERE wallpaper_name = ?');
...
if ($row['wp_count']==0) {
    ...
    $stmt = $FD->db()->conn()->prepare('INSERT INTO '.$FD->config('pref')."wallpaper (wallpaper_name, wallpaper_title, cat_id)
                 VALUES (?,
                         ?,
                         '".$_POST['catid']."')");
    ...
    for ($i=1; $i<=$_POST['options']; $i++)
    {
      ...
      if (isset($_FILES[$filesname]) AND $_POST['size'][$j] != '')
      {
        ...
        {
        case 0:
          $stmt = $FD->db()->conn()->prepare('INSERT INTO '.$FD->config('pref')."wallpaper_sizes (wallpaper_id, size)
                       VALUES ('".$wp_id."', ?)");
