...
{
    $index = $FD->db()->conn()->prepare('SELECT COUNT(*) FROM '.$FD->config('pref').'wallpaper WHERE wallpaper_name = ?');
    ...
    $update = $FD->db()->conn()->prepare('UPDATE '.$FD->config('pref')."wallpaper
               SET wallpaper_name = ?,
                   wallpaper_title  = ?,
                   cat_id      = '$_POST[catid]'
    ...
    // Update Files
        for ($i=0; $i<count($_POST['size']); $i++)
        {
            if (isset($_POST['delwp'][$i]) && $_POST['delwp'][$i])
            {
                ...
                $index = $FD->db()->conn()->query('SELECT size FROM '.$FD->config('pref')."wallpaper_sizes WHERE size_id = '".$_POST['delwp'][$i]."'");
                $size_name = $index->fetchColumn();
                $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."wallpaper_sizes WHERE size_id = '".$_POST['delwp'][$i]."'");
            ...
            else
            {
                ...
                if (isset($_FILES[$filesname]) && $_POST['wpnew'][$i]==1 && $_POST['size'][$i]!='' && !emptystr($_FILES["sizeimg_".$i]['tmp_name']))
                {
                    ...
                    {
                    case 0:
                        $insert = $FD->db()->conn()->prepare('INSERT INTO '.$FD->config('pref')."wallpaper_sizes (wallpaper_id, size)
                ...
                elseif ($_POST['wpnew'][$i]==0)
                {
                    ...
                    $index = $FD->db()->conn()->query('SELECT size FROM '.$FD->config('pref')."wallpaper_sizes WHERE size_id = '".$_POST['size_id'][$i]."'" );
                    $size_name = $index->fetchColumn();

                    image_rename('images/wallpaper/', $_POST['oldname'].'_'.$size_name, $_POST['oldname'].'_'.$_POST['size'][$i].'a');

                    $update = $FD->db()->conn()->prepare('UPDATE '.$FD->config('pref')."wallpaper_sizes
                               SET size = ?
     ...
     //Rename
     $index2 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$_POST[wallpaper_id]'");
...
elseif (isset($_POST['wallpaper_id']) AND isset($_POST['sended']) AND $_POST['sended'] == 'delete')
{
    ...
    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."wallpaper WHERE wallpaper_id = '$_POST[wallpaper_id]'");
    $wp_del_array = $index->fetch(PDO::FETCH_ASSOC);
    $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."wallpaper WHERE wallpaper_id = '$_POST[wallpaper_id]'");
    image_delete('images/wallpaper/', $wp_del_array['wallpaper_name'].'_s');

    $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$_POST[wallpaper_id]'");
    ...
    $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$_POST[wallpaper_id]'");
...
//////////////////////
  ...
  if ($_POST['wp_action'] == 'edit')
  {
    ...
    {
        $index = $FD->db()->conn()->query('SELECT wallpaper_name FROM '.$FD->config('pref')."wallpaper WHERE wallpaper_id = '$_POST[wallpaper_id]'");
        $wp_name = $index->fetchColumn();

        $index = $FD->db()->conn()->query('SELECT size FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$_POST[wallpaper_id]' LIMIT 1");
    ...
    $index2 = $FD->db()->conn()->query('SELECT COUNT(*) FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$_POST[wallpaper_id]'");
    $index2_num_rows = (int) $index2->fetchColumn();
    $index2 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$_POST[wallpaper_id]' ORDER BY size_id ASC LIMIT 1");
    $admin_sizes_arr = $index2->fetch(PDO::FETCH_ASSOC);
    $index2 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$_POST[wallpaper_id]' ORDER BY size_id ASC");
                        ...
                        <input type="hidden" name="oldname" value="'.$admin_wp_arr['old_name'].'" />
                            ...
                            <tr>
                                ...
                                <td class="config" valign="top">
                                    <select name="catid">
';
$index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'screen_cat WHERE cat_type = 2');
...
<table class="content" cellpadding="0" cellspacing="0">
    <tr><td colspan="4"><h3>Wallpaper l&ouml;schen</h3><hr></td></tr>
       ...
       <tr>
           ...
           $index2 = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."wallpaper_sizes WHERE wallpaper_id = '$wallpaper_arr[wallpaper_id]' ORDER BY size_id ASC");
           while ($sizes_arr = $index2->fetch(PDO::FETCH_ASSOC))
           {
             echo '<br>'.$sizes_arr['size'];
           }
           echo'</td>';
           $index2 = $FD->db()->conn()->query('SELECT cat_name FROM '.$FD->config('pref')."screen_cat WHERE cat_id = $wallpaper_arr[cat_id]");
...
////////////////////////
    ...
    if (isset($_POST['wpcatid']))
    {
        ...
        ';
        $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref')."wallpaper $wherecat ORDER BY wallpaper_id DESC");
