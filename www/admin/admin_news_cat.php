...
if ( $_POST['cat_id'] && $_POST['cat_action'] )
{
	...
	if ( $_POST['cat_action'] == 'edit' )
	{
		...
		echo'
									<span class="small">
										['.$FD->text("page", "'").' '.$news_config_arr['cat_pic_x'].' '.$FD->text("page", "'").' '.$news_config_arr['cat_pic_y'].' '.$FD->text("page", "'").'] ['.$FD->text("page", "'").' '.$news_config_arr['cat_pic_size'].' '.$FD->text("page", "'").']
									</span>
								</td>
							</tr>
		...
		// Last Category
		    ...
		    echo '
						<table class="configtable" cellpadding="4" cellspacing="0">
       						<tr>
           						<td class="config">
									<a class="link_button" href="?go=news_cat">'.$FD->text("page", "'").' '.$FD->text("page", "'").'</a>
								</td>
							</tr>
						</table>';
		}
...
elseif ( $showdefault == TRUE )
{
	...
	while ( $cat_arr = mysql_fetch_assoc ( $index ) )
	{
		...
		echo '
								...
								<td class="config" style="width: 100%;">
									#'.$cat_arr['cat_id'].' '.$cat_arr['cat_name'].' <span class="small">('.$FD->text("page", "'").' <b>'.$cat_arr['cat_user'].'</b> '.$FD->text("page", "'").' <b>'.date ( $global_config_arr['date'], $cat_arr['cat_date'] ).'</b>)</span><br>
