...
elseif ( $showdefault == TRUE )
{
	...
	while ( $cat_arr = mysql_fetch_assoc ( $index ) )
	{
		...
		echo '
								...
								<td class="config" style="width: 100%;">
									#'.$cat_arr['cat_id'].' '.$cat_arr['cat_name'].' <span class="small">('.$FD->text("page", "list_cat_created_by").' <b>'.$cat_arr['cat_user'].'</b> '.$FD->text("page", "list_cat_created_on").' <b>'.date ( $global_config_arr['date'], $cat_arr['cat_date'] ).'</b>)</span><br>
