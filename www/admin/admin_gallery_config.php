...
function secureData ( $DATA, $OUTPUT = FALSE ) {
    ...
    settype ( $DATA['img_mid_x'], 'integer' );
    settype ( $DATA['img_mid_y'], 'integer' );
    settype ( $DATA['img_small_x'], 'integer' );
    settype ( $DATA['img_small_y'], 'integer' );
...
if ( TRUE
    ...
    if ( FALSE !== $sql->updateData ( "gallery_config", $updateCols, $updateValues, "WHERE `id` = 1" ) ) {
        ...
        // Update failed
        $error = $sql->getError();
        $error = $TEXT['admin']->get("sql_error")." ".$error[1];
        systext( $TEXT['admin']->get("changes_not_saved")."<br>".$error, $TEXT["admin"]->get("error"), TRUE, $TEXT["admin"]->get("icon_save_error") );
    }
}
...
$adminpage->addText("img_mid_x",        $_POST['img_mid_x']);
$adminpage->addText("img_mid_y",        $_POST['img_mid_y']);
$adminpage->addText("img_small_x",      $_POST['img_small_x']);
