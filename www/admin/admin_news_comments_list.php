...
if (ACP_GO == "news_comments_list") {
  ...
  if (isset($_POST['commentid']) && isset($_POST['b8_action']))
  {
    ...
    $query = $FD->sql()->doQuery('SELECT comment_id, comment_title, comment_poster, '
                        .'comment_poster_id, comment_text, comment_classification '
                        .'FROM `{..pref..}news_comments` WHERE comment_id=\''.$_POST['commentid'].'\'');
    if ($result = mysql_fetch_assoc($query))
    {
      ...
      else
      {
        ...
        {
          $userindex = $FD->sql()->doQuery('SELECT user_name FROM `{..pref..}user` WHERE user_id = \''.$result['comment_poster_id'].'\'');
	    ...
	    else
	    {
	      ...
	      {
	        case 'mark_as_ham':
                 $query = $FD->sql()->doQuery('UPDATE `{..pref..}news_comments` SET comment_classification=\'1\' WHERE comment_id=\''.$_POST['commentid'].'\'');
                 if (!$query)
                 {
                   //MySQL error?
                   echo mysql_error();
                 ...
                 $query = $FD->sql()->doQuery('UPDATE `{..pref..}news_comments` SET needs_update=\'1\'');
	             break;
	        case 'mark_as_spam':
	             $query = $FD->sql()->doQuery('UPDATE `{..pref..}news_comments` SET comment_classification=\'-1\' WHERE comment_id=\''.$_POST['commentid'].'\'');
	             if (!$query)
	        ...
	        case 'unclassify':
	             if ($result['comment_classification']!=0)
	             {
	               $query = $FD->sql()->doQuery('UPDATE `{..pref..}news_comments` SET comment_classification=\'0\' WHERE comment_id=\''.$_POST['commentid'].'\'');
	               if ($result['comment_classification']>0)
	               {
  ...
  $update_query = $FD->sql()->doQuery('SELECT comment_id, comment_title, comment_poster, comment_poster_id, comment_text, '
                      .'IF(comment_poster_id=0, comment_poster, `{..pref..}user`.user_name) AS real_name '
                      .'FROM `{..pref..}news_comments` LEFT JOIN `{..pref..}user` '
                      .'ON `{..pref..}news_comments`.comment_poster_id=`{..pref..}user`.user_id '
  ...
  else
  {
    while ($row=mysql_fetch_assoc($update_query))
    {
      ...
      if (is_float($prob))
      {
        $FD->sql()->doQuery('UPDATE `{..pref..}news_comments` '
  ...
  if (isset($_REQUEST['b8_stats']))
  {
    ...
    $query = $FD->sql()->doQuery('SELECT token, count, LPAD(SUBSTRING(count, 1, LOCATE(\' \', count)-1), 10,\'0\') AS ham '
                        .'FROM b8_wordlist WHERE token NOT LIKE \'bayes*%\' '
                        .'ORDER BY ham DESC LIMIT 30');
    $ham_words = array();
    while ($result = mysql_fetch_assoc($query))
    {
    ...
    $query = $FD->sql()->doQuery('SELECT token, count, LOCATE(\' \', count) AS first_space, LOCATE(\' \', count, LOCATE(\' \', count)+1) AS second_space '
                        .'FROM b8_wordlist WHERE token NOT LIKE \'bayes*%\' '
                        .'ORDER BY LPAD(SUBSTRING(count, first_space+1, second_space-first_space-1),10,\'0\') DESC LIMIT 30');
    $spam_words = array();
    while ($result = mysql_fetch_assoc($query))
    {
    ...
    echo '</table>
    <br>
    <center><a href="?go=news_comments_list">Zur&uuml;ck zur Kommentarliste</a></center><br>';
  }//if stats requested
  else
  {
  ...
  $query = $FD->sql()->doQuery('SELECT COUNT(comment_id) AS cc FROM `{..pref..}news_comments`');
  ...
  //Kommentare auslesen
  $query = $FD->sql()->doQuery('SELECT comment_id, comment_title, comment_date, comment_poster, comment_poster_id, comment_text, comment_classification, '
                      .'`{..pref..}news_comments`.news_id AS news_id, `{..pref..}news`.news_id, news_title, '
                      .'IF(comment_poster_id=0, comment_poster, `{..pref..}user`.user_name) AS real_name, needs_update '
                      .'FROM `{..pref..}news_comments` LEFT JOIN `{..pref..}user` '
                      .'ON `{..pref..}news_comments`.comment_poster_id=`{..pref..}user`.user_id, `{..pref..}news` '
                      .'WHERE `{..pref..}news_comments`.news_id=`{..pref..}news`.news_id '
  ...
  if ($_GET['start']>0)
  {
    ...
    $prev_page = '<a href="?go=news_comments_list&amp;sort='.$_GET['sort'].'&amp;order='.$_GET['order'].'&amp;start='.$prev_start.'"><- zurück</a>';
  }//if nicht erste Seite
  //Ist dies nicht die letzte Seite?
  if ($_GET['start']+30<$cc)
  {
    $next_page = '<a href="?go=news_comments_list&amp;sort='.$_GET['sort'].'&amp;order='.$_GET['order'].'&amp;start='.($_GET['start']+30).'">weiter -></a>';
...
<?php
  echo '<a href="?go=news_comments_list&amp;sort=title&amp;order='.$inverse_order.'&amp;start='.$_GET['start'].'">Titel</a>';
?>
      </td>
      <td class="config" width="30%">
<?php
  echo '<a href="?go=news_comments_list&amp;sort=name&amp;order='.$inverse_order.'&amp;start='.$_GET['start'].'">Poster</a>';
?>
      </td>
      <td class="config" width="20%">
<?php
  echo '<a href="?go=news_comments_list&amp;sort=date&amp;order='.$inverse_order.'&amp;start='.$_GET['start'].'">Datum</a>';
?>
      </td>
      <td class="config" width="10%">
<?php
  echo '<a href="?go=news_comments_list&amp;sort=prob&amp;order='.$inverse_order.'&amp;start='.$_GET['start'].'">Spamwahrscheinlichkeit</a>';
?>
      </td>
      <td class="config" width="10%">
          bearbeiten
...
echo '             <form action="?go=news_comments_list" method="post" style="display:inline;">
             ...
             </form><form action="?go=news_comments_list" method="post" style="display:inline;">
               <input type="hidden" value="'.$_GET['start'].'" name="start">
               <input type="hidden" value="'.$_GET['sort'].'" name="sort">
               <input type="hidden" value="'.$_GET['order'].'" name="order">
               <input type="hidden" name="commentid" value="'.$comment_arr['comment_id'].'">
    ...
    else if ($comment_arr['comment_classification']>0)
    {
      ...
      echo '<font color="#008000" size="1">Als spamfrei markiert</font> <a href="'
          .'?go=news_comments_list&amp;b8_action=unclassify&amp;commentid='
...
?>
                            ...
                            <td colspan="3" style="text-align:center;" class="configthin">
                              <a href="?go=news_comments_list&amp;b8_stats=1">Statistik anzeigen</a>
                            </td>
                          </tr>
                        </table>
<?php
