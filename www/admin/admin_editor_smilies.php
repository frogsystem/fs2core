...
if (isset($_FILES['newsmilie']['name']) AND $_FILES['newsmilie']['name'] != '' AND isset($_POST['replace_string']))
{
    ...
    $FD->db()->conn()->exec('UPDATE '.$FD->config('pref')."smilies
                 SET `order`=`order`+1
                 WHERE `order`>$_POST[insert_after]");
    $stmt = $FD->db()->conn()->prepare('INSERT INTO '.$FD->config('pref')."smilies
...
elseif (isset($_POST['delete_smilies']))
{
    ...
    {
        $value = intval($value);
        $index = $FD->db()->conn()->query('SELECT id FROM '.$FD->config('pref')."smilies
                              WHERE `order`=$value");
        $id = $index->fetchColumn();

        $FD->db()->conn()->exec('DELETE FROM '.$FD->config('pref')."smilies
                     WHERE `order`=$value");
...
elseif (isset($_GET['action']) AND ($_GET['action']=='moveup' OR $_GET['action']=='movedown') AND isset($_GET['oid']))
{
    ...
    {
        $FD->db()->conn()->exec('UPDATE '.$FD->config('pref')."smilies SET `order`=0 WHERE `order`=$_GET[oid]");
        $FD->db()->conn()->exec('UPDATE '.$FD->config('pref')."smilies SET `order`=`order`+1 WHERE `order`=$_GET[oid]-1");
        $FD->db()->conn()->exec('UPDATE '.$FD->config('pref')."smilies SET `order`=$_GET[oid]-1 WHERE `order`=0");
    }
...
/////////////////////////
  ...
  $index = $FD->db()->conn()->query('SELECT COUNT(*) FROM '.$FD->config('pref').'smilies');
  $num_rows = $index->fetchColumn();
  $index = $FD->db()->conn()->query('SELECT * FROM '.$FD->config('pref').'smilies ORDER BY `order` ASC');
