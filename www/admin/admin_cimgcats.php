...
if(isset($_POST['add'])){
    ...
    if(!empty($name)){
        $stmt = $FD->db()->conn()->prepare('INSERT INTO `'.$FD->config('pref').'cimg_cats` (`name`, `description`) VALUES (?, ?)');
    ...
    if(isset($_POST['cat'])){
        $count = 0;
        $stmt = $FD->db()->conn()->prepare('UPDATE `'.$FD->config('pref').'cimg_cats` SET `name`=?, `description`=? WHERE `id`=?');
    ...
    if(isset($_POST['cat'])){
        $count = 0;
        $stmt = $FD->db()->conn()->prepare('UPDATE `'.$FD->config('pref')."cimg` SET `cat`=? WHERE `cat`=?");
        foreach($_POST['cat'] as $cat){
            $FD->db()->conn()->exec('DELETE FROM `'.$FD->config('pref').'cimg_cats` WHERE `id`='.intval($cat));
...
FS2_STRING;
$qry = $FD->db()->conn()->query('SELECT COUNT(*) FROM `'.$FD->config('pref').'cimg_cats`');
